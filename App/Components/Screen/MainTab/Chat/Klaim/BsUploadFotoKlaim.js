...
import utils_string from '../../../../../Utils/String';
...
class BsUploadFotoKlaim extends Component {
  constructor(props) {
    super(props);
    dataUser();
    this.state = {
      ...
      dataUpload: [
        {
          id: 0,
          title: 'Foto bagian yang mengalami kerusakan ',
          name: '',
          data: '',
  ...
  componentDidMount() {
    this.props.onUploadFotoKlaim(this);
  }
  componentWillUnmount() {
    this.props.onUploadFotoKlaim(undefined);
  }
  componentDidUpdate() {}
  ...
  setModalVisible(value, data_type) {
    console.log('setModalVisible', value);
    this.getLocation();
    this.RBSheet.open();
    this.setState({data_type});
    if (data_type == utils_string.data_type_klaim_kebakaran) {
      this.setState({
        type_polis: 'upload_foto_klaim_kebakaran',
        is_type_polis: true,
        dataUpload: [
          {
            id: 0,
    ...
    } else if (data_type == utils_string.data_type_klaim_kendaraan) {
      this.setState({
        type_polis: 'upload_foto_klaim_kendaraan',
        is_type_polis: true,
        dataUpload: [
          {
            id: 0,
            title: 'Foto SIM Pengemudi',
            name: '',
            data: '',
            path: require('./../../../../../assets/image/no_image.png'),
            uri: '',
    ...
    } else if (data_type == utils_string.data_type_klaim_kecelakaan) {
      this.setState({
        type_polis: 'upload_foto_klaim_personalaccident',
        is_type_polis: true,
        dataUpload: [
          {
            id: 0,
    ...
    } else if (data_type == utils_string.data_type_klaim_travel) {
      this.setState({
        type_polis: 'upload_foto_klaim_travel',
        is_type_polis: true,
        dataUpload: [
          {
            id: 0,
    ...
    if (value != null) {
      value.map((dataColumn, index) => {
        if (dataColumn.label == 'id_klaim') {
          this.setState({
            id_klaim: dataColumn.value,
            is_type_polis: true,
          });
        } else if (dataColumn.label == 'id_foto_klaim') {
          this.setState({
            id_foto_klaim: dataColumn.value,
          });
        }
      });
    }
  ...
  async validationForm() {
    this.state.dataUpload.map((value, index) => {
      if (
        ...
        this.getLocation();
        this.setState({errorHit: true, message: 'Harap nyalakan lokasi anda'});
        this.RBSheet.close();
        setTimeout(() => {
          this.RBSheetAllert.open();
        }, 500);
  ...
  async postFoto() {
    ...
    if (this.state.errorHit == false) {
      ...
      console.log('postFoto', this.state.data_type);

      switch (this.state.data_type) {
        case utils_string.data_type_klaim_kebakaran:
          console.log('postFoto keb', this.state.data_type);
          var param = {
            id_klaim: this.state.id_klaim,
            id_foto_klaim: this.state.id_foto_klaim,
            foto_kerusakan: this.state.dataUpload[0].data,
        ...
        case utils_string.data_type_klaim_kendaraan:
          console.log('postFoto ken', this.state.data_type);

          var param = {
            id_klaim: this.state.id_klaim,
            id_foto_klaim: this.state.id_foto_klaim,
            foto_sim: this.state.dataUpload[0].data,
            foto_stnk: this.state.dataUpload[1].data,
            foto_kerusakan: this.state.dataUpload[2].data,
            time_send_foto: moment().format('YYYY-MM-DD HH:mm:ss'),
            latitude_send_foto: this.state.latitude,
            longitude_send_foto: this.state.longitude,
          };
        ...
        case utils_string.data_type_klaim_kecelakaan:
          console.log('postFoto kec', this.state.data_type);
          ...
          var param = {
            ...
            foto_tiket: this.state.dataUpload[1].data,
            foto_kerusakan: this.state.dataUpload[2].data,
            time_send_foto: moment().format('YYYY-MM-DD HH:mm:ss'),
            latitude_send_foto: this.state.latitude,
            longitude_send_foto: this.state.longitude,
        ...
        case utils_string.data_type_klaim_travel:
          console.log('postFoto tra', this.state.data_type);
      ...
      await postDataWitHeader(
        Api.base_url2 + 'tripa/' + id + '/' + this.state.type_polis,
        param,
        tok,
      )
        .then(res => {
  ...
  eventClear() {
    ...
    this.RBSheet.close();
    this.props.sendToChat(this.state.id_klaim);
  }
  eventClose() {
    this.RBSheetAllert.close();
    setTimeout(() => {
...
export default BsUploadFotoKlaim;
