...
class CarouselBeritaHome extends Component {
  constructor(props) {
    super(props);
    this.state = {
      refresh_token: '',
  ...
  async componentDidMount() {
    ...
    this.firstAction();
    this.props.onCarouselBerita(this);
  }
  componentWillUnmount() {
    // this.props.navigation(undefined);
    this.props.onCarouselBerita(undefined);
  }
  ...
  getCarouselBerita() {
    console.log('getCarouselBerita');
    getDataWitHeader(
      Api.base_url2 + 'users/' + this.state.id_user + '/home/newsTripa',
      this.state.token,
    )
      .then(res => {
        if (res.success != false) {
          this.setState({
            data2: res.data.columns,
            visibleShimmer: false,
          });
          console.log('visibleShimmer ' + this.state.visibleShimmer);
        } else {
        }
      })
      .catch(err => {
...
export default CarouselBeritaHome;
